...
<section>
	<div class={cn('lg:fixed top-0 left-0 lg:h-[100vh] lg:w-full overflow-hidden flex justify-center items-center z-[6]', '')}>
		<div class="lg:flex justify-center items-center flex-col lg:size-full max-lg:container">
			...
			<div
				use:photogrid
				class="opacity-0 grid gap-8 lg:absolute lg:w-[150%] lg:h-[150%] lg:top-[50%] lg:left-[50%] lg:gap-0 lg:grid-cols-[repeat(50,2%)] lg:grid-rows-[repeat(50,2%)] lg:p-[60px] max-lg:!transform-none"
			>
				{#each data.gallery as item, i}
					<button
						data-photogrid-item
						class="relative flex justify-center items-center cursor-zoom-in max-lg:!transform-none max-lg:![grid-area:auto]"
						style={`grid-area: ${gridPositions[i]}`}
					>
